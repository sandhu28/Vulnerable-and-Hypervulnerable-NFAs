...
nfa LoopBack(nfa a, int pivot, char ch, int state)
{
    ...
    // q* is calculated as the minimum excludent (MEX) of a.q
    sort(a.q.begin(), a.q.end());
    int qStar = a.q.size();
...
nfa AnyLoopBack(nfa a, int qDash)
{
    ...
    for (auto &x : a.trans)
    {
        if (get<0>(x) == qDash)
        {
            ans.trans.insert(make_tuple(qStar, get<1>(x), get<2>(x)));
        }
    }
    return ans;
}
        ...
        for (int i = 0; i < vec.size(); i++)
        {
            for (int j = i + 1; j < vec.size(); j++)
            {
                ...
                for (auto &qDash : a.q)
                {
                    ...
                    nfa a2 = a;
                    a2.q.push_back(qi);
                    a2.trans.insert(make_tuple(qi, ch, vec[j]));
                    a2.q0 = qi;
                    a2.f.clear();
                    a2.f.insert(qDash);

                    nfa a3 = AnyLoopBack(a, qDash);

                    nfa as = a;
                    as.q0 = qDash;

                    nfa x1 = StateReduction(inter(a2, a3));
                    nfa x2 = StateReduction(inter(a1, x1));
                    nfa x3 = StateReduction(concat(ap, x2));
                    nfa x4 = StateReduction(concat(x3, comp(as)));
                    ans = (un(ans, x4));
                }
            }
        }
...
int main()
{
    ...
    cout << "Enter the number of characters in NFA language (l)"
         << "\n";
    int l;
    cin >> l;
    ...
    nfa out = StateReduction(AttackAutomaton(inp));
    PrintNFA(out);
